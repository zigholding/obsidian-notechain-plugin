...
export class Time{
	...
	parse_minutes(xt: string | number): { prefix: string; suffix: string; duration: number } {
		...
		const patterns = [
			...
			// 3. X小时Y分钟（带单位，「个」可省略）
			{ regex: /([零一二两三四五六七八九十\d]+)\s*个?\s*(?:小时|时)\s*([零一二两三四五六七八九十\d]+)\s*(?:分钟|分)/, handler: (m:string[]) => toNumber(m[1]) * 60 + toNumber(m[2]) },
			// 4. X小时（单独，「一个小时」等）
			{ regex: /([零一二两三四五六七八九十\d]+)\s*个?\s*(?:小时|时)(?!\s*半)/, handler: (m:string[]) => toNumber(m[1]) * 60 },
			// 5. X分钟（单独）
